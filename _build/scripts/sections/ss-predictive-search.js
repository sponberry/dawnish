import { debounce, getJsonFetchResponse, getNonLazyLoadedItem, getGADataItem, sendGA4Data } from "../plugins-imports/helpers";
// loading="true" results="true" open="true"
...
class PredictiveSearch extends SearchForm {
  ...
  onFocusOut() {
    setTimeout(() => {
      const oneActiveElement = this.contains(document.activeElement) || this.predictiveContainer.contains(document.activeElement);
  ...
  async getSearchspringSearchApiResults(searchTerm, apiService) {
    const queryString = `&q=${searchTerm}${apiService === 'search' || apiService === 'autocomplete' ? '&resultsFormat=json' : ''}`;
    const url = `${this.searchspringEndpoints.root}${this.searchspringEndpoints[apiService]}${queryString}${apiService === 'autocomplete' ? '&resultsPerPage=1000' : ''}`;
  ...
  async getSearchResults(searchTerm) {
    if (searchTerm !== null) {
      ...
      const matchingSearches = searchTerm.length > 2 ? theme.search.suggested_searches.filter(suggestion => suggestion.toLowerCase().startsWith(lowercaseTerm)) : [];

      const allSuggestions = [...matchingSearches, ...suggestedQueriesData.alternatives, {...suggestedQueriesData.suggested}];
      ...
      if (titleEl) {
        ...
        if (count) count.innerText = resultsData.results.length === 500 ? `${resultsData.results.length}+` : resultsData.results.length;
      ...
      fetch(`${routes.root_url}?section_id=searchspring-predictive-search`, { signal: this.abortController.signal })
        ...
        .then(async (text) => {
          const resultsMarkup = new DOMParser().parseFromString(text, "text/html").querySelector("#shopify-section-searchspring-predictive-search").innerHTML;
  ...
  addItemClickListeners(container) {
    ...
    async function handleClick(e) {
      ...
      e.preventDefault();
      const productItem = e.target.classList.contains('product-list-item') ? e.target : e.target.closest('.product-list-item');
      if (this.gaItems) {
        const gaItem = this.gaItems.find(item => String(item.item_id) === productItem.getAttribute('data-product-sku'));
